      ...
      <div className="w-1/2 sticky top-0 h-screen flex items-center justify-center p-12">
        <div className="relative w-full h-[85%] rounded-3xl overflow-hidden shadow-2xl">
          {sections.map((section, index) => (
            <div
              key={index}
              className={\`absolute inset-0 flex items-center justify-center transition-opacity duration-700 ease-in-out \${index === activeIndex ? 'opacity-100' : 'opacity-0 pointer-events-none'}\`}
            >
              <img
                src={section.image}
                alt={section.title}
              ...
              <div className="absolute bottom-12 left-12 text-white">
                <span className="text-sm font-medium text-white tracking-widest mb-2 block">
                  {\`0\${index + 1}\`}
                </span>
                <h3 className="text-4xl font-bold text-white mb-4">{section.title}</h3>
                <p className="text-xl text-white max-w-lg leading-relaxed">{section.content}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="h-screen w-full flex flex-col items-center justify-center px-20 snap-start">
          <div className="max-w-2xl">
            <div className="flex items-center justify-center mt-16">
              ...
              <span className="mx-6 text-white font-medium tracking-wider">SCROLL TO EXPLORE</span>
              <div className="w-16 h-1 bg-blue-500 rounded-full" />
            </div>
          </div>
        </div>
        ...
        {sections.map((section, index) => (
          ...
          >
            <div className={\`max-w-2xl transition-all duration-700 \${index === activeIndex ? 'opacity-100 translate-y-0' : 'opacity-50 translate-y-12'}\`}>
              ...
              <p className="text-xl text-white leading-relaxed">{section.content}</p>
            </div>
          </div>
        ))}
      </div>
