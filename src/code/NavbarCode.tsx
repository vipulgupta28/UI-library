  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black">
      ...
      >
              ...
              <ul className="flex space-x-8">
                {navItems.map((item) => (
                  ...
                  >
                    <a
                      href={item.href || '#'}
                      className={\`px-4 py-2 text-sm font-medium transition-all duration-300 flex items-center \${activeDropdown === item.id 
                    ...
                    >
                      ...
                      {item.dropdown && (
                        <svg
                          className={\`ml-2 h-4 w-4 transition-transform duration-300 \${activeDropdown === item.id ? 'rotate-180' : ''}\`}
        ...
        {hasDropdownItems && (
          <div 
            className={\`absolute top-full left-0 bg-white border-t rounded-xl border-gray-200 shadow-2xl transition-all duration-500 ease-out z-50 \${activeDropdown ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-4'}\`}
              ...
              <div className="relative">
                {navItems.map((item) => (
                  item.dropdown && (
                    <div 
                      key={item.id}
                      className={\`transition-all duration-500 ease-out \${activeDropdown === item.id 
